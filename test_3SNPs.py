...
for a in range(n_rep):
    ...
    b_gwas_sign_outer[a] = np.sign(np.outer(b[a], b[a]))

print('ld correlation coefficient\n', np.mean(ld, axis=0))
print('chi2 correlation coefficient\n', np.corrcoef(z ** 2, rowvar=False))
...
h2_ehe_sd1 = np.sqrt(np.sum(ld2_inv @ (sd_z2_outer * ld2 / n_gwas ** 2) @ ld2_inv, axis=(1, 2)))
h2_ehe_sd2 = np.sqrt(np.sum(ld2_inv @ (sd_z2_outer * np.abs(ld) / n_gwas ** 2) @ ld2_inv, axis=(1, 2)))
h2_ehe_sd3 = np.sqrt(np.sum(ld2_inv @ (sd_z2_outer * ld / n_gwas ** 2) @ ld2_inv, axis=(1, 2)))
